...
typedef struct {
  ...
  uint16_t hand_state
      [6]; //手指状态,0正在松开，1正在抓取，2位置到位停止，3力到位停止，5电流保护停止，6电缸堵转停止，7电缸故障停止
...
public:
  ...
  void Set_UDP_Configuration(int udp_cycle, int udp_port, int udp_force_coordinate, std::string udp_ip, bool hand,
                             bool rm_plus_base, bool rm_plus_state); //设置udp主动上报配置
  /*******************************运动控制回调函数******************************/
  ...
  void Arm_Movej_CANFD_Custom_Callback(
      rm_ros_interfaces::msg::Jointposcustom::SharedPtr msg); //角度透传控制高跟随下可自定义模式
  void Arm_Movep_CANFD_Callback(rm_ros_interfaces::msg::Cartepos::SharedPtr msg); //位姿透传控制
  void Arm_Movep_CANFD_Custom_Callback(
      rm_ros_interfaces::msg::Carteposcustom::SharedPtr msg); //位姿透传控制高跟随下可自定义模式
  ...
  void
  Arm_Set_Realtime_Push_Callback(const rm_ros_interfaces::msg::Setrealtimepush::SharedPtr msg); //设置主动上报配置参数
  /*******************************固件版本回调函数******************************/
  ...
  void Arm_Force_Position_Move_Joint_Callback(
      const rm_ros_interfaces::msg::Forcepositionmovejoint::SharedPtr msg); //力位混合透传（角度）
  void Arm_Force_Position_Move_Pose_Callback(
      const rm_ros_interfaces::msg::Forcepositionmovepose::SharedPtr msg); //力位混合透传（位姿）
...
private:
  ...
  rclcpp::Publisher<rm_ros_interfaces::msg::Sixforce>::SharedPtr
      Six_Zero_Force_Result; //六维力目标坐标系下系统受力发布器
  rclcpp::Publisher<rm_ros_interfaces::msg::Sixforce>::SharedPtr One_Force_Result; //一维力发布器
  rclcpp::Publisher<rm_ros_interfaces::msg::Sixforce>::SharedPtr
      One_Zero_Force_Result; //一维力目标坐标系下系统受力发布器
  ...
  rclcpp::Publisher<rm_ros_interfaces::msg::Rmplusbase>::SharedPtr
      Rm_Plus_Base_Result; //末端设备基础信息udp主动上报发布器
  rclcpp::Publisher<rm_ros_interfaces::msg::Rmplusstate>::SharedPtr
      Rm_Plus_State_Result; //末端设备实时信息udp主动上报发布器
  rclcpp::Publisher<rm_ros_interfaces::msg::Armcurrentstatus>::SharedPtr
      Arm_Current_Status_Result; //机械臂当前状态发布器
  ...
  rclcpp::Publisher<rm_ros_interfaces::msg::Jointposeeuler>::SharedPtr
      Joint_Pose_Euler_Result; //末端位姿欧拉角形式发布器
