 ...
 * @return int 函数执行的状态码。
            ...
            - -1: 数据发送失败，通信过程中出现问题。
            - -2:
 数据接收失败，通信过程中出现问题或者控制器超时没有返回。处理建议：1、联系睿尔曼公司技术支持确认控制器版本是否支持此功能；
            - -3: 返回值解析失败，接收到的数据格式不正确或不完整。
            - -4: 非六维力版本机械臂，不支持此功能。
 */
 ...
 *
 * @attention 如需配置连接ModbusTCP从站，则该接口需要在创建机械臂句柄后调用一次，否则在读写寄存器时无法接收到返回值。
 * @param handle 机械臂控制句柄
 * @param ip 从机IP地址
 * @param port 端口号
 ...
 * @param handle 机械臂控制句柄
 * @param mode 触觉传感器开关状态 0：关闭触觉传感器 1：打开触觉传感器（返回处理后数据）
 2：打开触觉传感器（返回原始数据）
 * @return int 函数执行的状态码。
            - 0: 成功。
            - 1: 控制器返回false，传递参数错误或机械臂状态发生错误。
 ...
 */
RM_INTERFACE_EXPORT rm_inverse_kinematics_all_solve_t
rm_algo_inverse_kinematics_all(rm_robot_handle *handle, rm_inverse_kinematics_params_t params);
 ...
 * @return int
 * 最优解索引，选解结果为ik_solve.q_solve[i],如果没有合适的解返回-1（比如求出8组解，但是8组都有关节角度超限位，那么就返回-1）
 */
 ...
 */
RM_INTERFACE_EXPORT int rm_algo_ikine_check_joint_velocity_limit(float dt, const float *const q_ref,
                                                                 const float *const q_solve);
 ...
 *              -3: 机型非RM75
 */
RM_INTERFACE_EXPORT int rm_algo_inverse_kinematics_rm75_for_arm_angle(rm_inverse_kinematics_params_t params,
                                                                      float arm_angle, float *q_solve);
 ...
 */
RM_INTERFACE_EXPORT void rm_algo_kin_set_singularity_thresholds(float limit_qe_algo, float limit_qw_algo,
                                                                float limit_d_algo);
/**
 ...
 */
RM_INTERFACE_EXPORT void rm_algo_kin_get_singularity_thresholds(float *limit_qe_algo, float *limit_qw_algo,
                                                                float *limit_d_algo);
