...
void RmArm::Arm_Set_Hand_Angle_Callback(const rm_ros_interfaces::msg::Handangle::SharedPtr msg) {
  ...
  // msg->hand_angle[1], msg->hand_angle[2], msg->hand_angle[3], msg->hand_angle[4], msg->hand_angle[5]); RCLCPP_INFO
  // (this->get_logger(),"Arm set hand data %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d\n", hand_data[0],
  // hand_data[1], hand_data[2], hand_data[3], hand_data[4], hand_data[5], hand_data[6], hand_data[7], hand_data[8],
  // hand_data[9], hand_data[10], hand_data[11]); res = Rm_Api.rm_write_registers(robot_handle, params, hand_data);
  // if(res == 0)
  // {
