...
void UdpPublisherNode::udp_timer_callback() {
  if (connect_state == 0) {
    ...
    // RCLCPP_INFO (this->get_logger(),"udp publisher arm_current_status is
...
auto UdpPublisherNode::read_data() -> bool {
  ...
  ssize_t numBytes =
      recvfrom(16, udp_socket_buffer, sizeof(udp_socket_buffer), 0, (struct sockaddr *)&clientAddr, &clientAddrLen);
  if (numBytes < 0) {
  ...
  std::cout << "Received from "       //<< inet_ntoa(clientAddr.sin_addr)
            << ":" << ntohs(clientAddr.sin_port) << ": " << udp_socket_buffer << std::endl;
  return (udp_socket_buffer[numBytes - 2] == 0X0D) && (udp_socket_buffer[numBytes - 1] == 0X0A);
}

UdpPublisherNode::UdpPublisherNode() : rclcpp::Node("udp_publish_node") {
  /*************************************************多线程********************************************/
  ...
  Udp_Timer = this->create_wall_timer(
      std::chrono::milliseconds(udp_cycle_g), [this]() { this->udp_timer_callback(); }, callback_group_time1_);
  /*****************************************************定时器*****************************************************************/
  ...
  Joint_Current_Result =
      this->create_publisher<rm_ros_interfaces::msg::Jointcurrent>("rm_driver/udp_joint_current", 10);
  Joint_En_Flag_Result = this->create_publisher<rm_ros_interfaces::msg::Jointenflag>("rm_driver/udp_joint_en_flag", 10);
  Joint_Pose_Euler_Result =
      this->create_publisher<rm_ros_interfaces::msg::Jointposeeuler>("rm_driver/udp_joint_pose_euler", 10);
  Joint_Speed_Result = this->create_publisher<rm_ros_interfaces::msg::Jointspeed>("rm_driver/udp_joint_speed", 10);
  Joint_Temperature_Result =
      this->create_publisher<rm_ros_interfaces::msg::Jointtemperature>("rm_driver/udp_joint_temperature", 10);
  Joint_Voltage_Result =
      this->create_publisher<rm_ros_interfaces::msg::Jointvoltage>("rm_driver/udp_joint_voltage", 10);
...
RmArm::RmArm() : rclcpp::Node("rm_driver") {
  ...
  /************************************************************************查询机械臂固件版本***************************************************************/
  // Get_Arm_Software_Version_Result =
  // this->create_publisher<rm_ros_interfaces::msg::Armsoftversion>("rm_driver/get_arm_software_version_result",
  // rclcpp::ParametersQoS()); Get_Arm_Software_Version_Cmd =
  // this->create_subscription<std_msgs::msg::Empty>("rm_driver/get_arm_software_version_cmd",rclcpp::ParametersQoS(),
