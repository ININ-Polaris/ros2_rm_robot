...
public:
  ...
  void GetArmOriginalState_Callback(
      const rm_ros_interfaces::msg::Armoriginalstate::SharedPtr msg); //机械臂状态原始结果回调函数
  void GetArmState_Callback(const rm_ros_interfaces::msg::Armstate::SharedPtr msg); //机械臂状态结果回调函数
  void
  GetArmSoftwareVersion_Callback(const rm_ros_interfaces::msg::Armsoftversion::SharedPtr msg); //控制器版本结果回调函数
...
private:
  ...
  rclcpp::Subscription<rm_ros_interfaces::msg::Armoriginalstate>::SharedPtr
      subscription_original_state; //关节原始状态订阅器
  rclcpp::Subscription<rm_ros_interfaces::msg::Armstate>::SharedPtr subscription_arm_state; //关节状态订阅器
  rclcpp::Subscription<rm_ros_interfaces::msg::Armsoftversion>::SharedPtr
      subscription_software_version;                                                         //控制器版本订阅器
  rclcpp::Subscription<rm_ros_interfaces::msg::Sixforce>::SharedPtr subscription_force_data; //六维力传感器状态订阅器
  rclcpp::Subscription<rm_ros_interfaces::msg::Sixforce>::SharedPtr
      subscription_zero_force_data; //六维力系统受力状态订阅器
  rclcpp::Subscription<rm_ros_interfaces::msg::Sixforce>::SharedPtr
      subscription_work_force_data; //六维力工作坐标系受力状态订阅器
  rclcpp::Subscription<rm_ros_interfaces::msg::Sixforce>::SharedPtr
      subscription_tool_force_data; //六维力工具坐标系受力状态订阅器
};
...
void GetArmState::GetArmState_Callback(const rm_ros_interfaces::msg::Armstate::SharedPtr msg) {
  ...
  RCLCPP_INFO(this->get_logger(),
              "pose state is: \n[position.x = %lf,\n position.y = %lf,\n position.z = %lf,\n orientation.x = %lf,\n "
              "orientation.y = %lf,\n orientation.z = %lf,\n orientation.w = %lf]\n",
              msg->pose.position.x, msg->pose.position.y, msg->pose.position.z, msg->pose.orientation.x,
...
void GetArmState::ForceData_Callback(const rm_ros_interfaces::msg::Sixforce::SharedPtr msg) {
  RCLCPP_INFO(this->get_logger(),
              "\nforce-force_fx is %f\nforce-force_fy is %f\nforce-force_fz is %f\nforce-force_mx is "
              "%f\nforce-force_my is %f\nforce-force_mz is %f\n ",
              msg->force_fx, msg->force_fy, msg->force_fz, msg->force_mx, msg->force_my, msg->force_mz);
}
/*******************************************************end**********************************************************/
/***********************************************机械臂六维力系统受力结果回调函数******************************************/
...
GetArmState::GetArmState() : rclcpp::Node("get_state") {
  subscription_original_state = this->create_subscription<rm_ros_interfaces::msg::Armoriginalstate>(
      "/rm_driver/get_current_arm_original_state_result", rclcpp::ParametersQoS(),
      std::bind(&GetArmState::GetArmOriginalState_Callback, this, _1));
  subscription_arm_state = this->create_subscription<rm_ros_interfaces::msg::Armstate>(
      "/rm_driver/get_current_arm_state_result", rclcpp::ParametersQoS(),
      std::bind(&GetArmState::GetArmState_Callback, this, _1));
  subscription_software_version = this->create_subscription<rm_ros_interfaces::msg::Armsoftversion>(
      "/rm_driver/get_arm_software_version_result", rclcpp::ParametersQoS(),
      std::bind(&GetArmState::GetArmSoftwareVersion_Callback, this, _1));
  subscription_force_data = this->create_subscription<rm_ros_interfaces::msg::Sixforce>(
      "/rm_driver/get_force_data_result", rclcpp::ParametersQoS(),
      std::bind(&GetArmState::ForceData_Callback, this, _1));
  subscription_zero_force_data = this->create_subscription<rm_ros_interfaces::msg::Sixforce>(
      "/rm_driver/get_zero_force_data_result", rclcpp::ParametersQoS(),
      std::bind(&GetArmState::ZeroForceData_Callback, this, _1));
  subscription_work_force_data = this->create_subscription<rm_ros_interfaces::msg::Sixforce>(
      "/rm_driver/get_work_force_data_result", rclcpp::ParametersQoS(),
      std::bind(&GetArmState::WorkForceData_Callback, this, _1));
  subscription_tool_force_data = this->create_subscription<rm_ros_interfaces::msg::Sixforce>(
      "/rm_driver/get_tool_force_data_result", rclcpp::ParametersQoS(),
      std::bind(&GetArmState::ToolForceData_Callback, this, _1));
  arm_state_publisher_ =
      this->create_publisher<std_msgs::msg::Empty>("/rm_driver/get_current_arm_state_cmd", rclcpp::ParametersQoS());
  arm_software_version_publisher_ =
      this->create_publisher<std_msgs::msg::Empty>("/rm_driver/get_arm_software_version_cmd", rclcpp::ParametersQoS());
  get_force_data_publisher_ =
      this->create_publisher<std_msgs::msg::Empty>("/rm_driver/get_force_data_cmd", rclcpp::ParametersQoS());
  get_arm_state();
}
/***********************************************end**************************************************/
