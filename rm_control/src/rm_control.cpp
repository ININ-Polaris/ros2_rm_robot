...
bool cubicSpline::getYbyX(double &x_in, double &y_out) {
  ...
  y_out = aa * y_sample_[klo] + bb * y_sample_[khi] +
          ((aa * aa * aa - aa) * M_[klo] + (bb * bb * bb - bb) * M_[khi]) * hh * hh / 6.0;
...
Rm_Control::Rm_Control(std::string name) : Node(name) {
  ...
  this->action_server_ = rclcpp_action::create_server<FollowJointTrajectory>(
      this, "/rm_group_controller/follow_joint_trajectory", std::bind(&Rm_Control::handle_goal, this, _1, _2),
      std::bind(&Rm_Control::handle_cancel, this, _1), std::bind(&Rm_Control::handle_accepted, this, _1));

  rclcpp::QoS qos(10);

  joint_pos_publisher = this->create_publisher<rm_ros_interfaces::msg::Jointpos>("/rm_driver/movej_canfd_cmd", qos);

  Get_Move_Stop_Cmd = this->create_subscription<std_msgs::msg::Bool>(
      "rm_driver/move_stop_cmd", rclcpp::ParametersQoS(),
      std::bind(&Rm_Control::get_move_stop_callback, this, std::placeholders::_1));
}

rclcpp_action::GoalResponse Rm_Control::handle_goal(const rclcpp_action::GoalUUID &uuid,
                                                    std::shared_ptr<const FollowJointTrajectory::Goal> goal) {
  std::cout << "---handle goal:" << goal->trajectory.joint_names.size() << std::endl;
  std::cout << goal->trajectory.header.frame_id.c_str() << goal->trajectory.header.stamp.sec
...
void Rm_Control::handle_accepted(const std::shared_ptr<GoalHandleFJT> goal_handle) {
  ...
  std::thread{std::bind(&Rm_Control::execute_move, this, _1), goal_handle}.detach();
}
...
void Rm_Control::execute_move(const std::shared_ptr<GoalHandleFJT> goal_handle) {
  ...
  if (point_num > 3) //判断当moveit规划的路点数大于3时为有效规划并进行三次样条插值
  {
    ...
    for (i = 0; i < point_num; i++) {
      ...
      time_from_start[i] =
          goal->trajectory.points[i].time_from_start.sec + goal->trajectory.points[i].time_from_start.nanosec / 1e9;
          ...
          if (spline.loadData(time_from_start, p_joint4, point_num, 0, 0, cubicSpline::BoundType_First_Derivative)) {
            ...
            if (spline.loadData(time_from_start, p_joint5, point_num, 0, 0, cubicSpline::BoundType_First_Derivative)) {
              ...
              if (spline.loadData(time_from_start, p_joint6, point_num, 0, 0,
                ...
                // joint7
                if (arm_type_ == 75) {
                  if (spline.loadData(time_from_start, p_joint7, point_num, 0, 0,
                                      cubicSpline::BoundType_First_Derivative)) {
                    p_joint7_.clear();
                    v_joint7_.clear();
                    a_joint7_.clear();
...
void Rm_Control::timer_callback() {
  ...
  if (point_changed) {
    if (p2.vector_cnt < p2.vector_len) {
      // RCLCPP_INFO(this->get_logger(), "Pos:[%f, %f, %f, %f, %f, %f]",  p_joint1_.at(p2.vector_cnt),
      // p_joint2_.at(p2.vector_cnt), p_joint3_.at(p2.vector_cnt), p_joint4_.at(p2.vector_cnt),
      // p_joint5_.at(p2.vector_cnt), p_joint6_.at(p2.vector_cnt));
      if (arm_type_ == 75) {
